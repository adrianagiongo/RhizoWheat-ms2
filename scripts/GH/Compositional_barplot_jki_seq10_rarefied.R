...
nsamples(psO_jki_seq10_rarefied)
psO_jki_seq10_rarefied

#plot histogram using "hist" function
hist(log10(taxa_sums(psO_jki_seq10_rarefied)))
...
##Aggregate rare taxa
#define total sums
total_samples_psO_jki_seq10_rarefied <- phyloseq::nsamples(psO_jki_seq10_rarefied)
total_sum_psO_psO_jki_seq10_rarefied = sample_sums(psO_jki_seq10_rarefied)

#aggregate based on phylum (detection higher than 40 copies and present in more than 85% of samples)
psO_jki_seq10_rarefied_aggreg_rare_phy <- aggregate_rare_taxa(psO_jki_seq10_rarefied, level="Phylum", detection = 600, prevalence =15/total_samples_psO_jki_seq10_rarefied)
psO_jki_seq10_rarefied_aggreg_rare_phy

##transform absolute to relative abundance using "transform" function
#for phylum
psO_jki_seq10_rarefied_aggreg_rare_phy_rel <- microbiome::transform(psO_jki_seq10_rarefied_aggreg_rare_phy, "compositional")
psO_jki_seq10_rarefied_aggreg_rare_phy_rel
...
#######
plot_bar_average_psO_jki_seq10_rarefied_aggreg_rare_phy_rel <- plot_composition(psO_jki_seq10_rarefied_aggreg_rare_phy_rel, otu.sort = "abundance", sample.sort = "Firmicutes", x.label = "Site_rot_tre", plot.type = "barplot", average_by = "Site_rot_tre", verbose = FALSE) +
  theme_bw() +
...
plot_bar_average_psO_jki_seq10_rarefied_aggreg_rare_phy_rel

ggsave("plot_bar_average_psO_jki_seq10_rarefied_aggreg_rare_phy_rel.png", path = "~/Documents/R_analysis/jki_seq10/output_jki_seq10/Compositional_barplot_jki_seq10/", width = 16, height = 15, units = "cm",dpi = 300, device = "png")
